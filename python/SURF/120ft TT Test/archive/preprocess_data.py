...
def unpackWaveData(file):
    ...
    post_sonar_entrance_index = (wave_data.iloc[sonar_lift_index:,0]<SONAR_ENTRANCE_HEIGHT).argmax()
    sonar_entrance_bracket_rows = wave_data.iloc[post_sonar_entrance_index-1:post_sonar_entrance_index+1,:].sort_values(by=['SonHt']) #The rows of the wave data array that bracket the SONAR entrance
    wave_sync_time = np.interp(SONAR_ENTRANCE_HEIGHT, sonar_entrance_bracket_rows['SonHt'],time[post_sonar_entrance_index-1:post_sonar_entrance_index+1])
...
def mergeData(sonar_df, wave_df):
    ...
    :param sonar_df: Pandas dataframe with wave data
    :param wave_df: Pandas dataframe with SONAR data
    '''
...
if __name__ == '__main__':
    ...
    while not (data_good == 'y' or data_good == 'Y'):
        ...
        ds = ds.assign_attrs({'wave_start_time': findWaveTrain(wave_df)})
        ds = ds.assign_attrs({'wave_end_time': findWaveTrain(wave_df)})
        wave_start_id = np.argmin(abs(ds['time'].to_numpy()-ds.attrs['wave_start_time'].astype('timedelta64[ms]')))
        wave_end_id = np.argmin(abs(ds['time'].to_numpy()-ds.attrs['wave_end_time'].astype('timedelta64[ms]')))

        sliced_wave_df = wave_df.iloc[wave_start_id:wave_end_id, :] #Discard data outside of user selected range
        
        #Perform fft on sea surface elevation data
        N = sliced_wave_df.shape[0]
        t_test = sliced_wave_df.index[-1] - sliced_wave_df.index[0] #total series time
        delta_t = (t_test/(N-1)).total_seconds() #time between samples

        #Ice Data FFT
        yf_ice = rfft(sliced_wave_df['IceWav'].to_numpy())[1:N//2]
        yf_sonar = rfft(sliced_wave_df['SonWav'].to_numpy())[1:N//2]
        ...
        #wave = StokesWave(1/top_xf, amp*2, TANK_DEPTH)
        wave = StokesWave(2, 0.2, TANK_DEPTH)
        wave_length = wave.L

        k = 2*np.pi/wave_length

        phase_speed = np.sqrt(wave_length*top_xf)
        ...
        t_plot = (t+sliced_wave_df.index[0].total_seconds())
        ax1.plot(sliced_wave_df.index.total_seconds(), sliced_wave_df['IceWav'])
        ax1.plot(t_plot,wave.calculateWave(0,t))

        ax2.plot(sliced_wave_df.index.total_seconds(), sliced_wave_df['SonWav'])
        print(ds.attrs['target_distance'])
        ax2.plot(t_plot,wave.calculateWave(ds.attrs['target_distance'],t))

        ax3.plot(xval := np.linspace(ds.attrs['target_distance'],0,5000), wave.calculateWave(xval,0))
    ...
    for i, time in enumerate(wave_df.index):
        z_sonar = wave_df['SonHt'].iloc[i] - ds.attrs['sonar_draft']
        z_ice = wave_df['IceHt'].iloc[i] - ds.attrs['target_draft']
        ...
        # plt.close()
        # plt.plot(x_val:=np.linspace(0,x_sonar,1000), sea_surface_height(x_val,time,waveProps))
    ...
    padded_in_sight = np.concatenate((np.full(wave_start_id, np.nan), np.array(in_sight), np.full(ds['time'].size - wave_end_id, np.nan)))
