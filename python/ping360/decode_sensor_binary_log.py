...
class PingViewerLogReader:
    ...
    def extract_times(self, timestamp):
        numerical_strings = re.findall('[0-9][0-9]*', timestamp)
        multipliers = [3600, 60, 1, 0.001]
        float_array = [float(n) for n in numerical_strings]
        timeFloat = 0
        for index, val in enumerate(float_array):
            timeFloat += val*multipliers[index]
    ...
    def __iter__(self):
        ...
        with open(self.filename, "rb") as file:
            ...
            # (timestamp, message) = self.unpack_message(file)
            # if self.extract_times(timestamp) != self.start_time_num:
            #     file.seek(current_position)
            #     search_ = b''
            #     amount_read = 0
            #     while not (match := re.search(
            #             self.start_time_str.encode(self.ENCODING), search_)):
            #         if match := self.TIMESTAMP_FORMAT.search(search_):
            #             if self.extract_times(match.group(0).decode(self.ENCODING)) > self.start_time_num:
            #                 #Seek location is in the last chunk
            #                 if amount_read > self.MAX_ARRAY_LENGTH:
            #                     file.seek(current_position+amount_read - 2*self.MAX_ARRAY_LENGTH)
            #                     amount_read -= self.MAX_ARRAY_LENGTH
            #                     search_ = file.read(self.MAX_ARRAY_LENGTH)
            #                 while search_:
            #                     if match := self.TIMESTAMP_FORMAT.search(search_):
            #                         if self.extract_times(match.group(0).decode(self.ENCODING)) > self.start_time_num:
            #                             break
            #                     search_ = search_[match.end():]
            #                 break
            #         search_= file.read(self.MAX_ARRAY_LENGTH)
            #         amount_read += self.MAX_ARRAY_LENGTH
            #         if not search_:
            #             raise EOFError('Timestamp match not found')
            #     # match was found
            #     start = match.start()
            #     timestamp = match.group(0).decode(self.ENCODING)
            #     amount_read -= (len(search_) - start)
            #     file.seek(current_position+amount_read-self.UINT.size)
    ...
    def parser(self, start_time: str = "00:00:00.000", message_ids: Set[int] = {1300, 2300, 2301}):
        ...
        self.start_time_num = self.extract_times(start_time)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file", help="File that contains PingViewer sensor log file.")
